...
if not _agentinbox_file.exists():
    ...
    print(f'[ERROR]   {{"url": "http://localhost:8080", "agent_id": "MyAgent", "agent_name": "MyAgent", "agent_description": "Description", "api_key": "optional-key"}}', file=sys.stderr)
    sys.exit(1)
...
Examples:
  ...
  python log_task.py \\
    ...
    # Resolve agent name and description from agents config if agent_id is provided
    agent_id = args.agent_id
    agent_name = args.agent_name
    agent_description = args.agent_description

    # If agent_id is provided and exists in agents config, use those values
    if agent_id in AGENTS_CONFIG:
        agent_config = AGENTS_CONFIG[agent_id]
        # Only override if not explicitly set via command line
        if args.agent_name == DEFAULT_AGENT_NAME:
            agent_name = agent_config.get("name", agent_name)
        if args.agent_description == DEFAULT_AGENT_DESCRIPTION:
