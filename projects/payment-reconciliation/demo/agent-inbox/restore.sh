...
docker exec "$INBOX_DB_CONTAINER" psql -U inboxuser -d postgres -c "DROP DATABASE IF EXISTS agent_inbox;"
docker exec "$INBOX_DB_CONTAINER" psql -U inboxuser -d postgres -c "CREATE DATABASE agent_inbox;"

# Restore backup
echo "Restoring backup..."
gunzip < "$BACKUP_FILE" | docker exec -i "$INBOX_DB_CONTAINER" psql -U inboxuser -d agent_inbox
